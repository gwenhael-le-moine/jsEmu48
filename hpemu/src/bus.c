    ...
    if (bus_info.ram_cfg &&
      ...
      if (bus_info.ce1_data) {
        ...
        write_map[SEG_OF(adr)] = NULL;
               ...
               ((bus_info.nce3_base ^ adr) & bus_info.nce3_size) == 0) {
      ...
      read_map[SEG_OF(adr)] =
          bus_info.rom_data +
          (adr & bus_info.rom_mask & (bus_info.da19 ? 0xFFFFF : 0x7FFFF));
      write_map[SEG_OF(adr)] = NULL;
    }
  }
  bus_info
      .map_cnt++; // Flag that pointers returned by bus_fast_peek may be invalid
}
