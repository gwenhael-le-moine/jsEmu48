...
void display_show() {
  ...
  if (shouldRender == TRUE) {
	...
	// Now you want to format the color to a correct format that SDL can
	// use. Basically we convert our RGB color to a hex-like BGR color.
...
void display_update(void) {
  ...
  if (!off_cnt) { /* Display is on */
    ...
    //LCD_Y+display_line_count*2, 131*2, 2);	unscare_mouse();	release_screen();
