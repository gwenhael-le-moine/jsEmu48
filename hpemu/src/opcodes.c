...
{
  if (!(opc[2] & 2)) {
    bus_write(cpu.reg[(opc[2] & 4) >> 1] + fs[opc[3]], cpu.d[opc[2] & 1],
              fl[opc[3]]);
  } else {
    bus_read(cpu.reg[(opc[2] & 4) >> 1] + fs[opc[3]], cpu.d[opc[2] & 1],
             fl[opc[3]]);
    cpu.cycles++;
  }
  cpu.pc += 4;
...
static void op81Af0(byte *opc) // Ri=r f/A
{
  ...
  int len = (opc[3] == 0xF) ? 5 : fl[opc[3]];
  reg_cpy(cpu.reg_r[i] + fs[opc[3]], cpu.reg[(opc[5] & 8) >> 2] + fs[opc[3]],
          len);
  cpu.pc += 6;
  cpu.cycles += 6 + len;
}
...
static void op81Af1(byte *opc) // r=Ri f/A
{
  int i = (opc[5] & 7) > 4 ? opc[5] & 3 : opc[5] & 7;
  int len = (opc[3] == 0xF) ? 5 : fl[opc[3]];
  reg_cpy(cpu.reg[(opc[5] & 8) >> 2] + fs[opc[3]], cpu.reg_r[i] + fs[opc[3]],
          len);
  cpu.pc += 6;
  cpu.cycles += 6 + len;
}
...
static void op81Af2(byte *opc) // rRiEX f/A
{
  int i = (opc[5] & 7) > 4 ? opc[5] & 3 : opc[5] & 7;
  int len = (opc[3] == 0xF) ? 5 : fl[opc[3]];
  reg_ex(cpu.reg[(opc[5] & 8) >> 2] + fs[opc[3]], cpu.reg_r[i] + fs[opc[3]],
         len);
  cpu.pc += 6;
  cpu.cycles += 6 + len;
}
...
static Opcode opcodes13[16] = {
    {op13a, NULL, "D0=A"},  {op13a, NULL, "D1=A"},   {op13a, NULL, "AD0EX"},
    {op13a, NULL, "AD1EX"}, {op13a, NULL, "D0=C"},   {op13a, NULL, "D1=C"},
    {op13a, NULL, "CD0EX"}, {op13a, NULL, "CD1EX"},  {op13b, NULL, "D0=AS"},
    {op13b, NULL, "D1=AS"}, {op13b, NULL, "AD0EXS"}, {op13b, NULL, "AD1EXS"},
    {op13b, NULL, "D0=CS"}, {op13b, NULL, "D1=CS"},  {op13b, NULL, "CD0EXS"},
    {op13b, NULL, "CD1EXS"}};
...
static Opcode opcodes9[16] = {{op9a, NULL, "?%u2%#2%z2 %F1, %G32YES %T32+3"},
